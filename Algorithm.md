...
* 좋은 해시 함수의 조건
	* 충돌이 적어야 한다.
	* 해시 함수 값이 해시 테이블에서 골고루 분포되어야 한다.
	* 계산이 빨라야 한다.
* 해시 함수 종류
	* 나눗셈법(Division Method)
		* h(k) = k mod M (M은 해시테이블의 크기이고, 소수이다.)
	* 곱셈법(Multiplication Method) 
		* h(k) = ⌊N(kA mod 1)⌋
		* A는 0 ~ 1 사이의 소수
		* kA mod 1은 k에 A를 곱해 나온 소수에서 소수점 이하 부분을 말한다.
		* kA mod 1에 N을 곱하므로 0 ~ N 사이의 값이 나온다.
		* A를 잘 잡는 것이 중요!
	* 폴딩
		* 키 값을 여러 부분으로 나눈 다음 값을 더하거나 XOR 연산을 수행한 결과물을 사용하는 방법이다.
	* 무작위법
		* 키 값을 난수로 생성해 사용하는 방법
	* 
	
## 해싱
* key를 해시 함수를 이용해 해시값으로 변환하는 과정
* 일반적으로 O(1)이 걸린다.
* 해시 함수의 성능과 해시 테이블 크기에 따라 성능이 갈린다.

## 해시 테이블
* key에 해시 함수를 적용해 생성한 인덱스와 매핑되는 value를 저장하는 자료구조
* 삽입/삭제/조회하는데 시간 복잡도는 O(1)이다.
* 무한한 데이터를 유한한 키값을 이용해 관리할 수 있다.
* 값이 저장되는 장소를 버킷이라고 한다.
* 버킷 안에는 여러개의 슬롯이 있을 수 있고, 한 슬롯에는 한 레코드만 저장이 가능하다.
* 오버플로우(Overflow) : 현재 인덱스의 버킷에 모든 슬롯을 사용해 저장할 공간이 없는 경우이다.
* 동의어(Synonym) : 충돌이 발생해 같은 해싱 결과 값을 가지는 레코드들이다.

## 해시 충돌
* 서로 다른 key가 동일한 인덱스 값(해시 함수의 적용 결과)을 가지는 경우이다.
	* 해결 방법
		* 체이닝
			* 버킷을 연결리스트로 관리하고, 효율성을 위해 연결리스트 맨앞에 새로운 값을 저장한다.
